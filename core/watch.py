...
# encoding: utf-8
#**************************************************************************************************
# Geert Dekkers Web Studio 2008 info@nznl.net
# nznl.com | nznl.net | nznl.org INTERNET PRODUCTIONS
# watch.py
#
# LN.image drag-and-drop content management system for LN.image, an Asset Management Web Application
...
# Watch works in concurrence with fix and generatekeywords. See ./script/install
# 
...
# 5. 	From paired .fla / .swf files, moves .swf to gallery. NOTE: 
# 
# Todo: Multipage PDF (exported packaged Indesign files) to JPG's 
#
...
# watch.py is normally started as a launchd item. See ./script/install and ./script/load
#
...
#
# 1. 	Open LN.settings, and check the database, user and password. Create if need be.
#
...
#
# 3. 	In LN.settings, change APP_CONTENT_ROOT to reflect the living quarters of your content. Or 
#		adversely, move your content to where APP_CONTENT_ROOT points to.
#
# 4. 	Edit APP_ROOT to match the path to your django app. Also, have a look at the other paths.
#
# 5. 	Run Fix, and leave it running. In short, Fix will fix filenames, renaming as needed to comply 
#		to workflow policy.
#		Please refer to the programmer's notes in Fix's header. Fix is set to a 60 second interval.
#		Change if needed. IMPORTANT NOTE: Before running, set images_imagecount.count to a five digit 
#		number, i.e. 10000. NOT to something like 00001: python will disregard the leading 0's. 
#
# 6.	Run "python watch.py". Your site will now be filled. This could take quite long, depending
# 		on your content. 
# 
# 7. 	Leave watch.py running in the background. While running, watch.py will watch over your content
#		as described above. Watch is set to rescan settings.APP_ROOT_CONTENT at a 360 second interval.
# 		This may be freely altered in settings.py.
#
# 8. 	Run generatekeywords. Schedule generatekeywords to run at regular intervals, i.e. using launchd. 
#		Once every 30 minutes should be OK. UPDATE: see ./script/install, ./script/load
#
#**************************************************************************************************
	...
	def convertImages(self,item,file_type='', mime_type=''):
			...
			if file_type:
				if mime_type:
					...
					logging.info('Converting items using mime type %s' % mime_type)
					...
					if mime_type.find('video') == 0:
						...
						c.ffmpeg(current_path, gallery_thumbs + fname.replace(os.path.splitext(fname)[1], '.flv'),'small')
						image_category = 'video'
					
					elif mime_type.find('audio') == 0:
						image_path = c.ffmpeg(current_path, gallery_images + fname.replace(os.path.splitext(fname)[1], '.flv'),'large')
						c.ffmpeg(current_path, gallery_thumbs + fname.replace(os.path.splitext(fname)[1], '.flv'),'cropped')
	 			...
	 			if item[(len(item)-5):(len(item)-4)] == "." or item[(len(item)-4):(len(item)-3)] == "." or item[(len(item)-3):(len(item)-2)] == ".": # Only files WITH extensions!
					...
					if image_LNID != '':
						...
						try:
							...
							if results[1] != '':  # Test for a value for convertImages, i.e. image_path
								try:
